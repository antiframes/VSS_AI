...
int angleToBall(int i){
  ...
  double px = env->currentBall.pos.x;
	double py = env->currentBall.pos.y;

  dx = px - env->home[i].pos.x;
  dy = py - env->home[i].pos.y;

	Fa.setXYMod(dx,dy,1); //m�dulo constante

  return Fa.ang();
}

#define LIMIT_LEFT 82
...
int chooseNextPoint(int angle, int i){
    ...
    int directionId =8;
    if (sqrt(pow(angle-angleToBall(i),2)) > 45) {
      directionId=newdir;
    }
    ...
    switch(directionId){
            ...
            case 4://leste
                ...
                if (env->home[i].pos.x<(LIMIT_RIGHT + DELTA))
                  dest[i][0]=LIMIT_RIGHT + DELTA+5;
                break;
            case 0://oeste
                dest[i][0] = env->home[i].pos.x + DELTA;
                if (env->home[i].pos.x>(LIMIT_LEFT - DELTA))
