...
std::string makeString(int team,int change, Environment * env){
        std::string s = "http://localhost:7777/send2?team=";
...
int Ai::sendToServer(int team,int change, Environment * env){
    CURL *curl;
    CURLcode res;
    curl = curl_easy_init();
    curl_easy_setopt(curl, CURLOPT_URL, makeString(team,change,env).c_str());
    ...
    //converter string de resposta em código de ação
    std::string::size_type sz;
    //int action = std::stoi (ans.ptr,&sz);

    int action = atoi(ans.ptr);
    return action;
}
